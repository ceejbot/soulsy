...
namespace config
{
	...
	bool MCMGlue::get_elden_demon_souls() { return true;
	}
	...
	RE::BSFixedString
		MCMGlue::get_form_name(RE::TESQuest*, const uint32_t a_index, const bool a_left, uint32_t a_position)
	{
		std::string form_string;
		if (const auto section = get_section_by_index(a_index, a_position); !section.empty())
		{
	...
	{
		auto elden = config::MCMGlue::get_elden_demon_souls();
						 ...
						 a_position == static_cast<uint32_t>(handle::position_setting::position_type::left)))
		{
			...
			auto max_pages = config::MCMGlue::get_max_page_count();
	...
	bool MCMGlue::registerGlue(RE::BSScript::IVirtualMachine* a_vm)
	{
