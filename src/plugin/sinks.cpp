...
void EquipEventSink::register_sink()
{
    RE::ScriptEventSourceHolder::GetSingleton()->AddEventSink(get_singleton());
}

EquipEventSink::event_result EquipEventSink::ProcessEvent(const RE::TESEquipEvent *a_event,
...
event_result KeyEventSink::ProcessEvent(RE::InputEvent *const *a_event,
                                        [[maybe_unused]] RE::BSTEventSource<RE::InputEvent *> *a_event_source)
{
    ...
    for (auto *event = *a_event; event; event = event->next)
    {
        ...
        auto is_toggle_key =
            common::is_key_valid_and_matches(key, key_binding->get_bottom_execute_or_toggle_action());
